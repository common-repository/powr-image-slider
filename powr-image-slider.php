<?php
    ...
    Plugin URI: https://www.powr.io/plugins/image-slider
    ...
    */
    ...
    class Powr_Image_Slider extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Image Slider.
        </p>
        <p>
    ...
    function powr_image_slider_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-image-slider' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-image-slider' label='$label'></div>";
      }else{
      	"<div class='powr-image-slider'></div>";
      }
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_image_slider_menu' );
    function powr_image_slider_menu() {
      add_menu_page( 'POWr Image Slider', 'POWr Image Slider', 'manage_options', 'powr-image-slider-settings', 'powr_image_slider_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_image_slider_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/image-slider';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/image-slider';
      }
    ...
    register_activation_hook(__FILE__, 'powr_image_slider_plugin_activate');
      add_action('admin_init', 'powr_image_slider_plugin_redirect');

      function powr_image_slider_plugin_activate() {
      add_option('powr_image_slider_plugin_do_activation_redirect', true);
      }
      ...
      function powr_image_slider_plugin_redirect() {
      if (get_option('powr_image_slider_plugin_do_activation_redirect', false)) {
          delete_option('powr_image_slider_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-image-slider-settings&' );
          }
       }
      }
